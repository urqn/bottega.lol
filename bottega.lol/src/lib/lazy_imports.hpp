...
#if defined(_DEBUG)
#define LIMPORT(routine) routine
#else
#define LIMPORT(name) ::li::detail::lazy_function<LAZY_IMPORTER_KHASH(#name), decltype(&name)>()
#define LIMPORT_DEF(name) ::li::detail::lazy_function<LAZY_IMPORTER_KHASH(#name), name>()
#define LIMPORT_MODULE(name) ::li::detail::lazy_module<LAZY_IMPORTER_KHASH(name)>()
#endif
